...
- 선택한 키 영역의 `?` 도움말에서 키코드 차이·ZMK 공식 호환성 표와 지연의 뜻·설정·사용 예시·해제 방법 안내
- Bluetooth, 마우스, 레이어 동작 분류 및 검색
- 모든 키에 선택적으로 지연 적용: 50~1000ms, 50ms 단위. 키캡에는 `(500ms)`처럼 시간만 표시
...
키를 선택한 뒤 **지연**을 체크하고 **50~1000ms**를 50ms 단위로 선택한 다음 **지연 적용**을 누르세요. 체크를 해제하고 적용하면 즉시 실행으로 돌아옵니다. 키 선택 목록·동작별 입력·원문 입력의 적용 버튼도 현재 지연 설정을 함께 적용합니다. 설정한 시간 이상 계속 눌러야 실행하며, 그 전에 떼면 아무 동작도 하지 않습니다. 실행 후에는 원래 동작처럼 키를 뗄 때 해제됩니다. 일반 키, 수정키, 미디어, 마우스, Bluetooth, 레이어 및 사용자 정의 동작에 사용할 수 있습니다. `&trans`는 실행할 동작이 없으므로 먼저 원하는 키나 동작을 선택하세요.

기존 부트로더·시스템 리셋의 500ms 바인딩도 지연 설정으로 읽어오며, 각 키의 시간을 독립적으로 변경하거나 해제할 수 있습니다. 키캡과 전체보기에는 원래 키 이름과 `(500ms)` 같은 짧은 시간 표시만 붙습니다. 시스템·전원 목록은 부트로더 진입과 시스템 재시작을 제공하고, 지연은 공통 옵션에서 선택합니다. ZMK의 `&bootloader`와 `&sys_reset`은 누른 키가 위치한 하프에만 적용됩니다.

설정은 같은 `modu.keymap` 안의 `tap-preferred` hold-tap 동작으로 저장합니다. 매개변수가 두 개인 동작은 press/release를 전달하는 매크로를 함께 생성합니다. 동일한 정의는 재사용하고, 시간 변경 시 다른 키가 공유하는 정의는 바꾸지 않습니다. 지연된 레이어 참조도 레이어 삭제 차단·번호 보정 대상입니다. 자세한 동작은 [ZMK Hold-Tap](https://zmk.dev/docs/keymaps/behaviors/hold-tap), [ZMK Macros](https://zmk.dev/docs/keymaps/behaviors/macros), [ZMK Reset Behaviors](https://zmk.dev/docs/keymaps/behaviors/reset)을 참고하세요.
...
실제 키맵 파싱, 바이트 단위 round-trip, LF/CRLF와 주석 보존, 단일 키 패치, 레이어 추가/복제/이름 변경/검증/삭제/참조 보정, 다섯 가지 키 드래그 작업, 기존 500ms 안전 홀드 호환성, 20단계 지연과 해제·재열기·공유 정의 독립성·두 매개변수 동작·지연된 레이어 참조, undo/redo, 공식 ZMK 키코드 카탈로그 및 영문명·기호·별칭 검색, 테마 설정 호환성, 가짜 빌드 프로세스의 성공/실패/취소를 검사합니다.
